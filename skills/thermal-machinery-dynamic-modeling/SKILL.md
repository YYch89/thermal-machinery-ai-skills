...
1. Identify the application, system boundary, working fluids, modeling depth, available data, and control objective.
2. Select or ask for the topology before equations: components, stream paths, split/merge points, heat-transfer pairings, pressure sources/losses, and environment boundaries.
3. Build a thermodynamic node and stream ledger. Every working fluid at every node must carry temperature, pressure, mass flow, molar flow when relevant, composition or quality, unit, basis, source, and calculation status.
4. Define component contracts: inputs, outputs, parameters, states, residuals, assumptions, and validation checks.
5. Build or inspect a design-point or steady solver before adding dynamics.
6. Map the design/steady result into dynamic initial conditions, lookup tables, controller references, and logged validation signals.
7. Add dynamic states only where physical storage or actuator/control dynamics require them.
8. Validate each stage before adding complexity: topology -> ledger -> component balances -> steady state -> dynamic initialization -> transient/control.
...
2. `thermodynamic_node_stream_ledger.csv`: node-by-node stream ledger with temperature, pressure, flow, composition or quality, units, source, and calculation status.
3. `component_contracts.csv`: each component's ports, equations, parameters, states, residuals, and checks.
4. `state_point_table.csv`: filtered design or steady state table derived from the ledger.
5. `design_to_dynamic_map.csv`: mapping from steady variables to dynamic states, initial conditions, lookup tables, and logged signals.
6. `initial_condition_audit.csv`: relevant integrators, delays, memories, transfer functions, thermal capacities, volume states, rotor states, and controller states.
7. `validation_report.md`: mass, species or element, energy, pressure, phase/composition, dynamic initialization, constraints, and source-provenance checks.
